...
{
	ASSERT(sGraphicsSystem != nullptr, "GraphicsSystem is not created");
...
void GraphicsSystem::Initialize(HWND window, bool fullscreen)
{
	...
	ASSERT(SUCCEEDED(hr), "GraphicsSystem failed to intialize device or swap chain");
...
void GraphicsSystem::BeginRender()
{
	...
	mImmediateContext->ClearDepthStencilView(mDepthStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.f, 0.f);
}
...
void GraphicsSystem::Resize(uint32_t width, uint32_t height)
{
	...
	ASSERT(SUCCEEDED(hr), "GraphicsSystem failed to creat render target");
