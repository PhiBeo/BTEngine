...
void BTEngine::Graphics::Texture::UnbindPS(uint32_t slot)
{
...
void Texture::Initialize(const std::filesystem::path& fileName)
{
	...
	ASSERT(SUCCEEDED(hr), "Texture -- failed to load texture %ls");
}
...
DXGI_FORMAT Texture::GetDXGIFormat(Format format)
{
	switch (format)
	{
	case BTEngine::Graphics::Texture::Format::RGBA_U8: return DXGI_FORMAT_R8G8B8A8_UNORM;
	case BTEngine::Graphics::Texture::Format::RGBA_U32: return DXGI_FORMAT_R32G32B32A32_UINT;
	}
